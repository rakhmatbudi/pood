...
public class PoodDatabase extends SQLiteOpenHelper {
    ...
    public List<Promo> getAllActivePromos() {
        ...
        Cursor cursor = null;
        try {
            cursor = db.query(TABLE_PROMOS, null, COLUMN_PROMO_IS_ACTIVE + " = ?",
                    new String[]{"1"}, null, null, COLUMN_PROMO_NAME + " ASC");
    ...
    public long saveOrderItemLocally(long orderId, CreateOrderItemRequest request) {
        ...
        try {
            ...
            values.put(COLUMN_ITEM_IS_SYNCED, 0);
    ...
    public void markOrderItemAsSynced(long localItemId, long serverItemId) {
        ...
        try {
            ...
            db.update(TABLE_ORDER_ITEMS, values, COLUMN_ITEM_ID + " = ?",
    ...
    public boolean hasMenuItems() {
        return getTableCount(TABLE_MENU_ITEMS) > 0;
    }

    public boolean hasMenuCategories() {
        return getTableCount(TABLE_CATEGORIES) > 0;
    }

    public boolean hasPromos() {
        return getTableCount(TABLE_PROMOS) > 0;
    }
    ...
    public Map<String, Integer> getAllTableCounts() {
        ...
        try {
            counts.put("menu_categories", getTableCount(TABLE_CATEGORIES));
            counts.put("menu_items", getTableCount(TABLE_MENU_ITEMS));
            counts.put("variants", getTableCount(TABLE_VARIANTS));
            counts.put("promos", getTableCount(TABLE_PROMOS));
            counts.put("order_types", getTableCount(TABLE_ORDER_TYPES));
            counts.put("order_statuses", getTableCount(TABLE_ORDER_STATUSES));
            counts.put("orders", getTableCount(TABLE_ORDERS));
            counts.put("order_items", getTableCount(TABLE_ORDER_ITEMS));
        } catch (Exception e) {
    ...
    public int getAllOrdersCount() {
        return getTableCount(TABLE_ORDERS);
    }

    public int getAllOrderItemsCount() {
        return getTableCount(TABLE_ORDER_ITEMS);
    }

    public int getAllVariantsCount() {
        return getTableCount(TABLE_VARIANTS);
    }
    ...
    public List<Long> getUnsyncedOrderIds() {
        ...
        try {
            Cursor cursor = db.query(TABLE_ORDERS, new String[]{COLUMN_ORDER_ID},
                    COLUMN_IS_SYNCED + " = ?", new String[]{"0"}, null, null,
    ...
    public void cleanupSyncedOrderItems(int daysOld) {
        ...
        try {
            ...
            db.delete(TABLE_ORDER_ITEMS, COLUMN_ITEM_IS_SYNCED + " = 1 AND " + COLUMN_ITEM_CREATED_AT + " < ?",
